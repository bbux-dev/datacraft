...
def test_load_spec_missing_type_defaults_to_values():
    ...
    loader = datagen.Loader(spec_missing_type)
...
def test_load_spec_undefined_refs():
    spec_undefined_refs = builder.spec_builder() \
        ...
        .build()
    loader = datagen.Loader(spec_undefined_refs)
    with pytest.raises(datagen.SpecException):
        loader.get('foo')
...
def test_shortcut_notation_config_in_key():
    ...
    loader = datagen.Loader(config_in_key_spec)
    supplier = loader.get('foo')
...
def test_registered_invalid_prepocessor():
    ...
    # for coverage
    datagen.loader.preprocess_spec(raw_spec)
