...
from datagen import builder, suppliers, Loader
...
def test_buffered_supplier_one_behind():
    ...
    buffered = datagen.supplier.common.BufferedValueSupplier(wrapped, buffer_size=3)
...
def test_buffered_supplier_more_than_one_behind():
    ...
    """
    wrapped = suppliers.values(builder.values(['a', 'b', 'c', 'd', 'e', 'f', 'g'], sample=True))
    buffered = datagen.supplier.common.BufferedValueSupplier(wrapped, buffer_size=4)
...
def test_buffered_supplier_size_one():
    ...
    """
    wrapped = suppliers.values(builder.values(['a', 'b', 'c', 'd', 'e', 'f', 'g'], sample=True))
    buffered = datagen.supplier.common.BufferedValueSupplier(wrapped, buffer_size=1)
