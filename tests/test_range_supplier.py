...
def test_ranges_missing_data():
    with pytest.raises(SpecException):
        range_handler.configure_range_supplier({'type': 'range'}, None)


def test_ranges_invalid_data_type():
    with pytest.raises(SpecException):
        range_handler.configure_range_supplier({'type': 'range', 'data': 42}, None)
...
def test_ranges_end_before_start():
    ...
    with pytest.raises(SpecException):
        range_handler.configure_range_supplier({'type': 'range', 'data': [start, end]}, None)
...
def test_ranges_valid():
    ...
    step = 2
    supplier = range_handler.configure_range_supplier({'type': 'range', 'data': [start, end, step]}, None)
...
def test_ranges_float_start_end():
    ...
    supplier = range_handler.configure_range_supplier({'type': 'range', 'data': [start, end]}, None)
