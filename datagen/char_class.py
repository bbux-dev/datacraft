...
from . import schemas, types, suppliers, utils
...
@types.registry.schemas(_CHAR_CLASS_KEY)
...
for key in _CLASS_MAPPING.keys():
    @types.registry.schemas("cc-" + key)
    def _get_char_class_alias_schema():
        """ get the schema for the char_class type """
        return schemas.load(_CHAR_CLASS_KEY)


@types.registry.types(_CHAR_CLASS_KEY)
def _configure_supplier(spec, _):
    ...
    if 'join_with' not in config:
        config['join_with'] = types.get_default('char_class_join_with')
...
@types.registry.types('cc-letters')
...
@types.registry.types('cc-printable')
...
@types.registry.types('cc-visible')
...
@types.registry.types('cc-punctuation')
...
@types.registry.types('cc-special')
...
@types.registry.types('cc-digits')
...
@types.registry.types('cc-hex-lower')
...
@types.registry.types('cc-hex-upper')
